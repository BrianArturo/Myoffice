...
if ($_POST["function"] == "add_document") {
  ...
  $SQL_ADD_DOCUMENTS  =   "INSERT INTO documents " .
    ...
    "";
  $modulo = 'documentos';
  $descrip = "Se agrego el siguiente archivo:" . $realname;
  $fecha = date("y/m/d h:i:s ");

  $SQL_BITACORA = "INSERT INTO bitacora (id_usuario, modulo, fecha, id_caso, descripcion)"
    . "VALUES ('" . $_SESSION["auth_id"] . "','" . $modulo . "',' " . $fecha . "', '" . $caso_id . "', '" . $descrip . "')";

  $mysqli3->real_query($SQL_ADD_DOCUMENTS);
  $mysqli3->real_query($SQL_BITACORA);
  $mysqli3->close();
...
if ($_POST["function"] == "buscar") {

  if ($_POST["name"] == null || $_POST["name"] == "") {
    $SQL_DOCUMENTS = "SELECT * FROM  documents   where documents.caso_id=" . $caso_id . " order by extention";
  } else {
    $SQL_DOCUMENTS = "SELECT * FROM  documents   where documents.caso_id=" . $caso_id . " AND realname rlike '" . $_POST["name"] . "' order by 		extention";
  }
  // echo $SQL_DOCUMENTS;
  $mysqli2->real_query($SQL_DOCUMENTS);
  $contacts = $mysqli2->use_result();
...
if ($_POST["function"] == "delete_document" and is_numeric($_POST["file_id"])) {
  ...
  $f = $files->fetch_assoc();
  $_FILE_TO_DELETE = "../download/" . $f["caso_id"] . "/" . $f["name"];
  #echo $_FILE_TO_DELETE;
  #echo "il file essiste ".$_FILE_TO_DELETE;
  $realname = $f[realname];
  $fecha = date("y/m/d h:i:s ");
  $descrip_e = "Se Elimino el siguiente archivo:" . $realname;
  $modulo = 'documentos';
  $SQL_BITACORA = "INSERT INTO bitacora (id_usuario, modulo, fecha, id_caso, descripcion)"
    . "VALUES ('" . $_SESSION["auth_id"] . "','" . $modulo . "',' " . $fecha . "', '" . $caso_id . "', '" . $descrip_e . "')";
